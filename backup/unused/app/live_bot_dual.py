...
def add_technical_indicator(
    df, atr_window: int = 4, atr_multiplier: float = 2.4, rsi_window: int = 12, sma_window: int = 4
):
    df_copy = df.copy()
...
print(Style.BRIGHT + f"KEY1 = {API_KEY1}, SECRET1 = {API_SECRET1}")
API_KEY2 = os.getenv("API_KEY2")
API_SECRET2 = os.getenv("API_SECRET2")
print(Style.BRIGHT + f"KEY1 = {API_KEY2}, SECRET1 = {API_SECRET2}")
...
balance1 = print_user_and_wallet_info(client=client1)
asset_balance1 = get_asset_balance(balance=balance1, asset=asset)
currency_balance1 = get_asset_balance(balance=balance1, asset=currency)

client2 = Client(API_KEY2, API_SECRET2)
balance2 = print_user_and_wallet_info(client=client2)
asset_balance2 = get_asset_balance(balance=balance2, asset=asset)
currency_balance2 = get_asset_balance(balance=balance2, asset=currency)

exchange_info = client1.get_exchange_info()
...
df = fetch_initial_candles(client=client1, symbol=symbol, interval=interval)
...
socket_thread = threading.Thread(
    ...
    args=(data_queue, stop_event, symbol, interval, API_KEY1, API_SECRET1),
    daemon=True,
)
socket_thread.start()
...
print("CLIENT 1")
print(f" Simbolo: {symbol} ({asset_price_balance1}), holding: {holding1}")
print(f" {currency} disponibili: {currency_balance1}")
print("CLIENT 2")
print(f" Simbolo: {symbol} ({asset_price_balance2}), holding: {holding2}")
print(f" {currency} disponibili: {currency_balance2}")
...
while True:
    ...
    if len(df_copy) > 1:
        ...
        # if not holding1 and sum_buy >= num_cond:
        if sum_buy >= num_cond:
            if not holding1:
                # procedo all'acquisto su Client 1
                print(
                    Style.BRIGHT + Fore.GREEN + f"Buy Signal detected at {current_candle_time} "
                    f"and price {current_candle_price} (Client 1)"
                )
            ...
            if not holding2:
                # procedo all'acquisto su Client 2
                print(
                    Style.BRIGHT + Fore.GREEN + f"Buy Signal detected at {current_candle_time} "
                    f"and price {current_candle_price} (Client 2)"
                )
                response = proceed_buy(
                    client=client2,
                    asset=asset,
        ...
        if sum_sell >= num_cond:
            if holding1:
                # procedo alla vendita su Client 1
                print(
            ...
            if holding2:
                # procedo alla vendita su Client 2
                print(
                    Style.BRIGHT + Fore.RED + f"Sell Signal detected at {current_candle_time} "
                    f"and price {current_candle_price} (Client 2)"
                )
